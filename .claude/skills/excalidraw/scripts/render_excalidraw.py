...
Usage (from the skill's `scripts/` directory):
    python render_excalidraw.py <path-to-file.excalidraw> [--output path.png] [--scale 2] [--width 1920]

First-time setup (from the skill's `scripts/` directory):
...
def render(
    ...
    try:
        ...
        print("Run from the skill's scripts/ directory: pip install -r requirements.txt && playwright install chromium", file=sys.stderr)
        sys.exit(1)
    ...
    with sync_playwright() as p:
        try:
            ...
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                ...
                print("Run from the skill's scripts/ directory: playwright install chromium", file=sys.stderr)
                sys.exit(1)
            raise
