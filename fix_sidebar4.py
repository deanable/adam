...
if old2 in content:
    ...
    count += 1
    print("OK: Standalone mapper")
else:
    old2b = "                Id = c.Id, Name = c.Name, ParentId = c.ParentId, AssetCount = c.AssetCount\n            }).ToList()"
    new2b = "                Id = c.Id, Name = c.Name, ParentId = c.ParentId, AssetCount = c.AssetCount, IsSmart = c.IsSmart\n            }).ToList()"
    if old2b in content:
        content = content.replace(old2b, new2b, 1)
        count += 1
        print("OK: Standalone mapper (LF)")
    else:
...
if old3 in content:
    ...
    if old3b in content:
        ...
        count += 1
        print("OK: Broker mapper (LF)")
    else:
        print("FAIL: Broker mapper not found")

# ====== Change 4: _isSmart field in CollectionNode ======
...
# ====== Change 5: IsSmart property + SmartIcon ======
...
if missing:
    print(f"\nWARNING: Missing from file: {missing}")
else:
    print("\nAll properties verified present")
