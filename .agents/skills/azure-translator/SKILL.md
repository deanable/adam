...
|----------|-------|-------------|
| Troubleshooting | L30-L34 | Diagnosing and fixing common Azure Translator issues in Foundry Tools, plus understanding HTTP status/error codes and their causes and resolutions. |
| Best Practices | L36-L44 | Best practices for custom translation: managing containers, glossaries, dataset splits, Foundry/custom models, and document translation usage, FAQs, and operational guidance. |
| Decision Making | L46-L53 | Guidance on choosing standard vs Custom Translator, evaluating custom models with BLEU, and migrating apps between Translator API versions and platforms. |
| Limits & Quotas | L55-L65 | Limits, formats, and data size requirements for Custom Translator training/documents, character and request quotas, and language/locale support for Translator and Document Translation. |
| Security | L67-L78 | Configuring Azure Translator security: encryption, auth (keys, Entra ID, managed identities, SAS), VNets/firewalls, secure workspaces, and protecting data in document translation. |
| Configuration | L80-L89 | Configuring Azure Translator behavior: resource setup, Docker/container settings, profanity filters, content exclusion, dynamic dictionaries, word alignment, and monitoring usage metrics. |
| Integrations & Coding Patterns | L91-L129 | Using Translator REST/SDK/containers for text & document translation: endpoints, params, async jobs, status, formats, custom models, preview features, Power Automate, and dictionary/transliteration APIs. |
| Deployment | L131-L136 | Running Translator in Docker containers and deploying or copying custom translation models across regions and Foundry projects for scalable, portable translation setups. |
